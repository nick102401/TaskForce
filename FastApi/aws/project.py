...
class Project(PersonalHomepage):
    ...
    def create_status(self, statusName, projectName, statusColor='', statusType=0, bugFlag=0, filterType='filter',
                      userName=env.USERNAME_PM):
        """
        创建任务状态或BUG状态
        ...
        if not statusColor:
            statusColor = 'Red'
        statusColor = self.config.get_color(statusColor)

        # 获取项目ID
        projectId = self.query_project_id_by_name(projectName, filterType=filterType, userName=userName)

        # 获取状态序列号
        resp = self.query_status(projectName=projectName, bugFlag=bugFlag, filterType='filter',
                                 userName=env.USERNAME_PM)
        statusList = resp['content']['data']['list']
        serial = len(statusList) + 1
        ...
        if bugFlag == 1:
            del data['statusType']
        url = '/api/task/case/task/projects/{0}/status'.format(projectId)
        resp = req_exec(method, url, data=data, username=userName)
        return resp
    ...
    def delete_status(self, statusName, projectName, bugFlag=0, filterType='filter', userName=env.USERNAME_PM):
        """
        删除任务状态/BUG状态
        :param statusName: 名称
        :param projectName: 项目名称
        ...
        """
        # 获取任务状态ID
        statusBody = self.query_status_info_by_name(statusName=statusName, projectName=projectName, bugFlag=bugFlag,
                                                    filterType=filterType, userName=userName)
        projectId = statusBody['projectId']
...
class Task(Project):
    ...
    def create_task(self, taskName, broTaskName='', description='', deadLine='', taskGetDateLine='', points=1,
        ...
        priorityId = get_value_from_resp(resp['content'], 'taskPriorityId', 'priorityName', priorityName)
    ...
    def delete_tasks(self, taskNameList=None, ifDeleteAll=False, userName=env.USERNAME_PM):
        """
        删除任务
        :param taskNameList: 任务名称
        :param ifDeleteAll: True:删除当前项目所有未完结任务
        :param userName: 默认为PM角色
        :return:
        """
        if taskNameList is None:
            taskNameList = []

        if ifDeleteAll:
            taskNameList = []
            resp = self.query_tasks(archive=0, userName=userName)
            taskList = resp['content']['data']['list']
            for task in taskList:
                taskNameList.append(task['title'])
...
class Plan(Project):
    ...
    def create_task_for_plan(self, taskName, broTaskName='', description='', deadLine='', taskGetDateLine='', points=1,
                             countedPoints=1, priceFlag=False, priorityName='普通', typeName='开发任务', statusName='未开发',
                             bugStatusName='轻微', executor='', planName='', userName=env.USERNAME_PM):
        """
        创建计划任务
        :param taskName: 任务名称
        :param broTaskName: 被关联任务名称
        ...
        resp = self.task.create_task(taskName, broTaskName=broTaskName, description=description, deadLine=deadLine,
                                     ...
                                     priceFlag=priceFlag, priorityName=priorityName, typeName=typeName,
                                     statusName=statusName, bugStatusName=bugStatusName, executor=executor,
                                     planId=planId, userName=userName)
        return resp
...
class Personnel(Project):
    ...
    def query_persons(self, userName=env.USERNAME_PM):
        """
        查询人员
        :param userName: 默认为PM角色
        :return:
    ...
    def delete_members(self, del_userNameList=None, ifDeleteAll=False, userName=env.USERNAME_PM, userId=True):
        """
        删除项目成员
        :param del_userNameList: 项目成员列表
        :param ifDeleteAll: 是否删除所有
        :param userName:
        :param userId:
        :return:
        """

        if del_userNameList is None:
            del_userNameList = []

        if ifDeleteAll:
            del_userNameList = []
            resp = self.query_persons(userName=userName)
            memberList = resp['content']['data']['list']
            for member in memberList:
                del_userNameList.append(member['realName'])

        for del_userName in del_userNameList:
            self.delete_member(del_userName, userName=userName, userId=userId)
...
if __name__ == '__main__':
    ...
    # tm.delete_tasks(ifDeleteAll=True)
    ...
    # person = Personnel('接口测试20210914')
    # person.add_member(memberName=env.USERNAME_QA, roleName='项目管理')
    # person.delete_members(ifDeleteAll=True)
    # person.give_red_flower(memberName=env.USERNAME_RD, remark='ssssssssssss1')
    # person.create_recruit(postName='中文测试1234', postSum=10, postJobShare=50, postType=4, postDescription='中文测试1234',
    # person                startTime='2021-8-6', endTime='2021-8-6')
    # person.modify_recruit(postName='CICS', newPostName='中文测试1234', postSum=100, postJobShare=20, postType=6,
    # person                postDescription='CICS', startTime='2021-8-10', endTime='2021-8-10')
    # person.operate_recruit(postName='中文测试1234', openFlag=True)
    # person.delete_recruit(postName='中文测试1234')
    # person.query_recruits()
    # try:
    #     person.query_recruit_info_by_name('test123')
