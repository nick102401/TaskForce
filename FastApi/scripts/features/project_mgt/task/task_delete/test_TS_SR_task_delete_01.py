...
/*
@author:王东
@file:test_TS_SR_task_delete_01.py
@time:2021/08/23
*/
...
def test_step_01():
    ...
    1.任务删除成功
    '''
    resp = task.query_tasks(assign=0)
    assert taskName in json.dumps(resp['content'])

    resp = task.delete_task(taskName=taskName, assign=0)
    assert resp['retCode'] == 200
    assert resp['content']['msg'] == 'success'
...
def test_step_02():
    ...
    resp = task.delete_task(taskName=subtaskName, assign=0)
    assert resp['retCode'] == 200
    assert resp['content']['msg'] == 'success'
